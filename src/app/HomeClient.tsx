...
import { ArrowRight, Code2, PenTool, TerminalSquare, Calendar } from "lucide-react";
    ...
    return (
        <div className="container mx-auto max-w-5xl px-6 py-20 flex flex-col items-center justify-center min-h-[85vh]">
            ...
            <motion.div
                ...
                className="w-full mt-32 relative text-left"
            >
                <div className="flex items-center justify-between mb-8">
                    <h2 className="text-2xl font-bold flex items-center gap-2 text-zinc-900 dark:text-white">
                        <Code2 className="text-blue-500 w-6 h-6" /> 최신 포스트
                    </h2>
                    <Link href="/posts" className="text-sm font-medium text-blue-500 hover:text-blue-600 dark:hover:text-blue-400 flex items-center gap-1 group">
                        모두 보기 <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                    </Link>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {latestPosts.map((post) => (
                        <Link href={`/posts/${post.slug}`} key={post.slug} className="group block p-6 rounded-3xl bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 hover:border-blue-500/50 dark:hover:border-blue-400/50 transition-all duration-300 shadow-sm hover:shadow-xl hover:shadow-blue-500/10 h-full flex flex-col">
                            <div className="flex items-center justify-between mb-4 text-xs">
                                <time className="text-zinc-500 dark:text-zinc-400 font-medium flex items-center gap-1">
                                    <Calendar size={12} /> {post.date}
                                </time>
                            </div>
                            <h3 className="text-lg font-bold mb-2 text-zinc-900 dark:text-white group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors line-clamp-2">
                                {post.title}
                            </h3>
                            <p className="text-zinc-600 dark:text-zinc-400 text-sm line-clamp-2 flex-grow">
                                {post.description}
                            </p>
                        </Link>
                    ))}
            ...
            <motion.div
                ...
                className="w-full grid grid-cols-1 md:grid-cols-3 gap-6 mt-20 relative"
            >
