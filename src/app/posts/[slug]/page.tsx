...
export default async function PostPage({ params }: Params) {
    ...
    return (
        <article className="container mx-auto max-w-3xl px-6 py-20 pb-32">
            <Link href="/posts" className="inline-flex items-center text-sm font-medium text-zinc-500 hover:text-blue-500 mb-8 transition-colors group">
                ...
                <div className="flex items-center gap-6 text-zinc-500 dark:text-zinc-400 text-sm">
                    ...
                    <div className="flex gap-2">
                        {(post.tags as unknown as string[] || []).map((tag: string, i: number) => (
                            <span key={i} className="text-blue-500 font-medium">#{tag}</span>
                        ))}
                    </div>
                </div>
                <div className="mt-4">
